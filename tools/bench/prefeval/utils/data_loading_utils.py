...
def load_turns_data(dataset_dir: str):
    with open(
        f"{dataset_dir}/benchmark_dataset/filtered_inter_turns.json", "r"
    ) as infile:
...
def update_task_data_implicit(
    task, end_generation, zero_shot_response=None, revised_messages=None, critic=None
):
    if zero_shot_response is not None:
        task["zero_shot_response_to_q"] = zero_shot_response
        task["response_to_q"] = revised_messages
